...
void *philosopher_rutine(void *input)
{

    struct filo_data_s *filo_data = input;

    for (int i = 0; i < 7; i++)
    {
        meditating(filo_data->filo_id, filo_data->filo_status);
        eating(filo_data->filo_id, filo_data->filo_status, filo_data->forks);
    }
    filo_data->filo_status[filo_data->filo_id] = 'd';
    printf("Filozof %d: [%c] -- Skoncyl dzialanie\n",
           filo_data->filo_id, filo_data->filo_status[filo_data->filo_id]);
    return 0;
}
